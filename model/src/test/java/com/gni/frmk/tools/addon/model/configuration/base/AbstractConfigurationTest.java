 ...
 * @author: e03229
 */
public abstract class AbstractConfigurationTest<T extends Configuration<?>> {
    ...
    private static final Package CONFIGURATION_BASE_PACKAGE = BaseConfiguration.class.getPackage();
    ...
    protected AbstractConfigurationTest(Class<T> configurationType) {
        type = configurationType;
        final Package componentPackage = configurationType.getPackage();
        xmlRule = new XmlTestRule(COMPONENT_PACKAGE, COMPONENT_BASE_PACKAGE, CONFIGURATION_BASE_PACKAGE,COMPONENT_TEST_PACKAGE, componentPackage);
    ...
    protected abstract T buildConfiguration();

    @Test
    public void testToXml() throws JAXBException, IOException, SAXException {
        T configuration = buildConfiguration();
    ...
    public void testFromXml() throws JAXBException, IOException, SAXException {
        ...
        T configuration = xmlRule.load(in, type);
        T expected = buildConfiguration();

        ReflectionAssert.assertReflectionEquals(expected, configuration);
